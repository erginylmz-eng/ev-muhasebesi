/* Yılmaz Hane Defteri — ev muhasebesi uygulaması
   Üç farklı barındırma ortamını destekler (aşağıdaki BACKEND değişkenine bakınız):
   1) Claude Artifact — paylaşılan aile verisi Claude'un "artifact" runtime
      yeteneği ile HTML içine gömülü olarak saklanır.
   2) Firebase (ör. GitHub Pages üzerinde eski kurulum) — window.__FIREBASE_CONFIG__
   ...
   3) Kendi sunucu ("selfhosted", 10 Eylül 2026'da eklendi) — window.__SELFHOSTED__
      tanımlıysa aktif olur: Firebase'e HİÇ bağımlı değil. Kimlik yine Google
      ile Giriş ama doğrudan Google'ın kendi OAuth 2.0 servisiyle (server/
      klasöründeki küçük Node/Express backend üzerinden); veri de Firestore
      yerine bu backend'in tuttuğu düz JSON dosyalarında saklanır. Firebase ve
      selfhosted modları, kullanıcıya aynı deneyimi (Google ile Giriş, aile
      üyeleri arasında paylaşılan veri, davet, otomatik yedekleme) sunar; bu
      yüzden aşağıda ikisini de kapsayan bir HAS_REAL_AUTH bayrağı kullanılıyor
      ve "fb*" adlı fonksiyonlar (isimleri tarihsel nedenlerle korunuyor)
      içeride BACKEND'e göre Firestore SDK'sı ile mi yoksa /api/... çağrılarıyla
      mı konuşacaklarına karar veriyor. */
(function(){
  "use strict";
  ...
  var BACKEND = (typeof window !== 'undefined' && window.__FIREBASE_CONFIG__) ? 'firebase' :
    (typeof window !== 'undefined' && window.__SELFHOSTED__) ? 'selfhosted' : 'artifact';
  /* Firebase ve selfhosted, "gerçek Google kimlik doğrulaması + aile üyeleri
     arasında paylaşılan sunucu tarafı veri" anlamında birbirinin aynısı
     davranır; UI kodunun çoğu bu ikisini ayırt etmeye gerek duymaz. */
  var HAS_REAL_AUTH = (BACKEND === 'firebase' || BACKEND === 'selfhosted');
  ...
  var ICONS = {
    ...
    bell: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M6 8a6 6 0 0112 0c0 4.5 1.5 6 2 7H4c.5-1 2-2.5 2-7z"/><path d="M10 20a2 2 0 004 0"/></svg>',
    coin: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M9.5 9.2a2.6 2.6 0 015-.4M9.5 14.8a2.6 2.6 0 005-.4"/><path d="M12 7v10"/></svg>',
    insight: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2v3"/><path d="M12 19v3"/><circle cx="12" cy="12" r="6"/><path d="M12 9v3l2 2"/></svg>'
  };
  ...
  var ui = {
    ...
    reminderForm: null, // { id, title, date, repeatYearly, note } — Ayarlar > Hatırlatıcılar formu
    finRates: null,     // Finansal Bilgilendirme: docs/rates.json'dan çekilen { usdTry, eurTry, gramAltinTry, onsAltinUsd, updatedAt }
    finRatesError: null,
    finRatesLoading: false,
    backupsList: null   // Ayarlar > Veri > Yedekler: [{id, savedAt}] — istenince Firestore'dan çekilir
  };
  try{
  ...
  } else if(BACKEND === 'selfhosted'){
    shBoot();
  }

  /* ---------------- kendi sunucu (selfhosted) — Firebase'siz eşdeğer ----------------
     Kimlik ve veri artık Firestore SDK'sı üzerinden değil, server/server.js'in
     sunduğu küçük REST API'siyle (/api/...) konuşuluyor. Firestore'un
     onSnapshot() ile anlık bildirdiği değişiklikler burada basit bir periyodik
     "GET /api/state" yoklamasıyla (polling) taklit ediliyor — küçük bir hane
     defteri (birkaç kişi) için fazlasıyla yeterli, WebSocket gibi ekstra bir
     altyapı gerektirmiyor. fbUser/fbBooting/fbError değişkenleri Firebase'le
     BİREBİR AYNI şekilde dolduruluyor ki geri kalan tüm arayüz kodu (topbar,
     Ayarlar, giriş ekranları) hangi backend olduğunu bilmeden çalışabilsin. */
  var shPollTimer = null;
  var shLastKnownUpdatedAt = null;
  var shSaving = false; /* kendi POST'umuz sürerken poll'un aynı anda state'i değiştirmesini önler */

  function shBoot(){
    fetch('/api/me', { credentials:'same-origin' }).then(function(r){ return r.json(); }).then(function(me){
      if(!me.loggedIn){ fbUser = null; fbBooting = false; render(); return; }
      fbUser = { uid: me.user.uid, email: me.user.email, displayName: me.user.name, photoURL: me.user.photoURL };
      return fetch('/api/state', { credentials:'same-origin' }).then(function(r2){
        if(r2.status === 401){ fbUser = null; fbBooting = false; render(); return; }
        if(!r2.ok) throw new Error('HTTP ' + r2.status);
        return r2.json().then(function(data){
          state = migrateState(data.state || defaultState());
          if(!state.people.some(function(p){ return p.id === fbUser.uid; })){
            state.people.push({ id: fbUser.uid, name: fbUser.displayName || fbUser.email, approved: true, isAdmin: false });
          }
          localIdentity = fbUser.uid;
          shLastKnownUpdatedAt = data.updatedAt;
          fbBooting = false;
          render();
          shStartPolling();
        });
      ...
      fbError = (err && err.message) || 'Sunucuya bağlanılamadı.';
      fbBooting = false;
      render();
    });
  }

  function shStartPolling(){
    if(shPollTimer) return;
    shPollTimer = setInterval(function(){
      if(shSaving || document.hidden) return;
      fetch('/api/state', { credentials:'same-origin' }).then(function(r){
        if(r.status === 401){
          clearInterval(shPollTimer); shPollTimer = null;
          fbUser = null; state = null; render();
          return;
        }
        if(!r.ok) return;
        return r.json().then(function(data){
          if(data.updatedAt === shLastKnownUpdatedAt) return;
          shLastKnownUpdatedAt = data.updatedAt;
          state = migrateState(data.state || defaultState());
          render();
        });
      }).catch(function(){ /* geçici ağ hatası — bir sonraki yoklamada tekrar denenir */ });
    }, 5000);
  }

  /* persist()'in selfhosted dalı bunu çağırır (bkz. aşağıdaki persist()). */
  function shPersist(){
    if(!state) return;
    shSaving = true;
    fetch('/api/state', {
  ...
  function fbSignIn(){
    if(BACKEND === 'selfhosted'){ window.location.href = '/api/auth/google/start'; return; }
    if(!fbAuth){ return; }
  ...
  function fbSignOut(){
    if(BACKEND === 'selfhosted'){
      if(shPollTimer){ clearInterval(shPollTimer); shPollTimer = null; }
  ...
  /* Ayarlar'daki "Davet Et" formu bunu çağırır: household id'sini invites/{email}
     ...
     aynı household'a katılır (bkz. fbResolveHousehold / server.js'teki resolveHousehold). */
  function fbInvite(email){
    email = (email || '').trim().toLowerCase();
    if(!email) return Promise.reject(new Error('geçersiz e-posta'));
    if(BACKEND === 'selfhosted'){
    ...
    if(!fbHouseholdId || !fbDb) return Promise.reject(new Error('geçersiz e-posta'));
  ...
  /* ---------------- Otomatik periyodik yedekleme ----------------
     Her başarılı kayıttan sonra çağrılır; günde en fazla bir kez (tarayıcı
     tarih değiştiğinde) households/{id}/backups/{YYYY-MM-DD} dokümanına o
     anki TAM state anlık görüntüsünü yazar — üzerine yazma yaparak aynı gün
     içinde tek doküman kalır. Hiçbir dış servise/Claude'a bağımlı değildir,
     tamamen istemci + Firestore üzerinde otomatik çalışır. 30 günden eski
     yedekler arka planda sessizce budanır (depolama sınırsızca büyümesin diye). */
  var _lastBackupDate = null;
  function fbSaveDailyBackupIfNeeded(){
    if(BACKEND!=='firebase' || !fbHouseholdRef || !state) return;
    var today = todayISO();
    if(_lastBackupDate === today) return;
    _lastBackupDate = today;
    fbHouseholdRef.collection('backups').doc(today).set({
      state: state,
      savedAt: firebase.firestore.FieldValue.serverTimestamp()
    }).then(function(){ fbPruneOldBackups(); }).catch(function(){ /* yedek başarısız olsa da ana kayıt zaten tamam; sessizce geç */ });
  }
  function fbPruneOldBackups(){
    if(!fbHouseholdRef) return;
    var cutoff = addDaysToDate(todayISO(), -30);
    fbHouseholdRef.collection('backups').get().then(function(snap){
      snap.forEach(function(doc){
        if(doc.id < cutoff) doc.ref.delete().catch(function(){});
      });
    }).catch(function(){});
  }
  /* Ayarlar > Veri > Yedekler'de "Yedekleri Görüntüle"ye basınca çağrılır. */
  function fbListBackups(){
    if(BACKEND === 'selfhosted'){
      ui.backupsList = 'loading';
      render();
      fetch('/api/backups', { credentials:'same-origin' }).then(function(r){ return r.json(); }).then(function(data){
        ui.backupsList = data.backups || [];
        render();
      }).catch(function(err){
        ui.backupsList = [];
        showToast('Yedek listesi alınamadı: ' + (err && err.message ? err.message : 'bilinmeyen hata'));
      });
      return;
    }
    if(!fbHouseholdRef) return;
    ui.backupsList = 'loading';
    render();
    fbHouseholdRef.collection('backups').orderBy(firebase.firestore.FieldPath.documentId(), 'desc').limit(30).get().then(function(snap){
      var list = [];
      snap.forEach(function(doc){ list.push({ id: doc.id }); });
      ui.backupsList = list;
      render();
    }).catch(function(err){
      ui.backupsList = [];
  ...
  function fbRestoreBackup(backupId){
    if(BACKEND === 'selfhosted'){
      fetch('/api/backups/' + encodeURIComponent(backupId), { credentials:'same-origin' }).then(function(r){
        if(!r.ok) throw new Error('HTTP ' + r.status);
        return r.json();
      }).then(function(data){
        state = migrateState(data.state || defaultState());
        logAudit('backup-restore', backupId + ' tarihli yedek geri yüklendi');
        persist();
        showToast(backupId + ' tarihli yedek geri yüklendi.');
      }).catch(function(err){
        showToast('Yedek geri yüklenemedi: ' + (err && err.message ? err.message : 'bilinmeyen hata'));
      });
      return;
    }
    if(!fbHouseholdRef) return;
    fbHouseholdRef.collection('backups').doc(backupId).get().then(function(doc){
      if(!doc.exists){ showToast('Yedek bulunamadı.'); return; }
      state = migrateState(doc.data().state || defaultState());
      logAudit('backup-restore', backupId + ' tarihli yedek geri yüklendi');
      persist();
      showToast(backupId + ' tarihli yedek geri yüklendi.');
    }).catch(function(err){
  ...
  function budgetStatusForMonth(monthKeyStr){
    ...
    /* Abonelikler (madde: "Abonelik paneli"): bu ay vadesi gelmiş ama henüz
       kaydedilmemiş abonelik tutarları, ilgili kategorinin bütçesine "beklenen"
       olarak ekleniyor — kullanıcı "Ekle"ye basmadan önce bile bütçe çubuğunda
       görünsün diye. dueRecurring() zaten o ay için henüz oluşturulmamış olanları
       döndürüyor, bu yüzden gerçek kayıt oluşunca çift sayılmıyor. */
    var pendingSubByCategory = {};
    var monthCur = monthKeyStr === currentMonthKey();
    if(monthCur){
      dueRecurring().forEach(function(r){
        if(!r.isSubscription || r.type!=='expense') return;
        pendingSubByCategory[r.categoryId] = (pendingSubByCategory[r.categoryId]||0) + r.amount;
      });
    }
    var out = [];
    state.categories.expense.forEach(function(c){
      ...
      var pendingSub = pendingSubByCategory[c.id] || 0;
      var pct = limit>0 ? ((spent+pendingSub)/limit*100) : 0;
      var level = pct>=100 ? 'over' : (pct>=80 ? 'warn' : 'ok');
      out.push({ cat:c, limit:limit, spent:spent, pendingSub:pendingSub, pct:pct, level:level });
    });
    out.sort(function(a,b){ return b.pct-a.pct; });
    return out;
  }

  /* ---------------- Abonelikler ("Abonelik paneli") ----------------
     Bir düzenli ödeme (recurring), formundaki "Bu bir abonelik" kutusu
     işaretlenerek abonelik sayılır (yalnızca gider tipi). Süresiz olduğu için
     zaten cashFlowProjection()'daki indefiniteRecurringNet'e dahil olup gelecek
     aylara yansıyor; budgetStatusForMonth() de bu ayki henüz eklenmemiş
     tutarını ilgili kategori bütçesine "beklenen" olarak ekliyor. */
  function activeSubscriptions(){
    return state.recurring.filter(function(r){ return r.active && r.isSubscription && r.type==='expense'; })
      .sort(function(a,b){ return b.amount-a.amount; });
  }
  function subscriptionsMonthlyTotal(){
    return activeSubscriptions().reduce(function(s,r){ return s + r.amount; }, 0);
  }

  /* ---------------- Harcama alışkanlığı içgörüleri ----------------
     Kural tabanlı (yapay zeka gerektirmeyen), mevcut Raporlar verisinden
     türetilen basit tespitler: bu ayki kategori harcamalarını geçen ayla
     karşılaştırır. Gürültüyü azaltmak için yalnızca anlamlı tutar (>=100 TL
     fark) ve anlamlı oran (%|15| ve üzeri, ya da yeni bir kategori) değişimi
     gösterilir; en büyük TL farkına sahip en fazla 4 içgörü döner. */
  function spendingInsights(monthKeyStr){
    var prevMk = addMonths(monthKeyStr, -1);
    var curRows = categoryBreakdown(monthKeyStr, 'expense');
    var prevRows = categoryBreakdown(prevMk, 'expense');
    var prevByCat = {};
    prevRows.forEach(function(r){ prevByCat[r.cat.id] = r.amount; });
    var out = [];
    curRows.forEach(function(r){
      var cur = r.amount;
      var prev = prevByCat[r.cat.id] || 0;
      var diff = cur - prev;
      if(Math.abs(diff) < 100) return;
      if(prev <= 0){
        out.push({ diff: diff, text: 'Bu ay ilk kez ' + esc(r.cat.name) + ' kategorisinde harcama yaptınız: ' + fmtTRY(cur) + '.', kind:'new' });
        return;
      }
      var pct = (diff/prev*100);
      if(Math.abs(pct) < 15) return;
      var yon = diff>0 ? 'daha fazla' : 'daha az';
      out.push({ diff: Math.abs(diff), text: esc(r.cat.name) + ' kategorisinde geçen aya göre %' + Math.abs(Math.round(pct)) + ' ' + yon + ' harcadınız (' + (diff>0?'+':'−') + fmtTRY(Math.abs(diff)) + ').', kind: diff>0?'up':'down' });
    });
    /* toplam gider karşılaştırması, varsa her zaman ilk sırada */
    var curTotal = totalsForMonth(monthKeyStr).expense;
    var prevTotal = totalsForMonth(prevMk).expense;
    if(prevTotal > 0 && curTotal > 0){
      var totalDiff = curTotal - prevTotal;
      var totalPct = (totalDiff/prevTotal*100);
      if(Math.abs(totalPct) >= 10 && Math.abs(totalDiff) >= 200){
        var totalYon = totalDiff>0 ? 'daha fazla' : 'daha az';
        out.unshift({ diff: Infinity, text: 'Bu ay toplamda geçen aya göre %' + Math.abs(Math.round(totalPct)) + ' ' + totalYon + ' harcadınız (' + (totalDiff>0?'+':'−') + fmtTRY(Math.abs(totalDiff)) + ').', kind: totalDiff>0?'up':'down' });
      }
    }
  ...
  /* ---------------- Tarih bazlı genel hatırlatıcılar ----------------
     state.reminders: { id, title, date (YYYY-MM-DD, ilk/tek seferlik tarih),
     repeatYearly, note }. Yıllık tekrar işaretliyse, bu yılki gün geçmişse
     bir sonraki yıla döner; işaretli değilse tarih olduğu gibi kullanılır. */
  function nextReminderOccurrence(r){
    if(!r.repeatYearly) return r.date;
    var today = todayISO();
    var md = r.date.slice(5); // "AA-GG"
    var thisYear = today.slice(0,4) + '-' + md;
    return thisYear >= today ? thisYear : (parseInt(today.slice(0,4),10)+1) + '-' + md;
  }
  ...
  /* Önümüzdeki 7 gün içinde (bugün dahil, geçmişse de hâlâ "gördün mü" diye)
     olan genel hatırlatıcılar — kredi kartı ödeme hatırlatıcıları ayrı
     fonksiyonda (creditCardDueSoonReminders). */
  function upcomingReminders(){
    return state.reminders.map(function(r){
      var occ = nextReminderOccurrence(r);
      return { reminder: r, date: occ, days: daysUntil(occ) };
    }).filter(function(x){ return x.days <= 7 && x.reminder.dismissedOccurrence !== x.date; })
      .sort(function(a,b){ return a.days-b.days; });
  }
  /* Her kredi kartının bu ayki ödeme gününe 3/2/1 gün kala (ve tam ödeme
     gününde) hatırlatıcı üretir — yalnızca ödenmemiş (pozitif) borcu varsa. */
  function creditCardDueSoonReminders(){
    ...
    state.accounts.filter(function(a){ return a.type==='card' && a.statementDay && a.paymentDay; }).forEach(function(a){
      ...
      var days = daysUntil(payDate);
      if(days < 0 || days > 3) return;
      var bal = accountBalance(a.id);
      if(bal <= 0) return;
      out.push({ account: a, date: payDate, days: days, balance: bal });
    });
    out.sort(function(a,b){ return a.days-b.days; });
    return out;
  }

  /* ---------------- Gelecek Ödemeler (Raporlar) ----------------
     Kullanıcı isteği: hangi gün ne kadar ödemesi olduğunu tek bir kronolojik
     tabloda, kümülatif bir bakiyeyle birlikte görebilmek (Excel'de tuttuğu
     tabloya benzer). Dört kaynaktan besleniyor: (1) planlı [gelecek tarihli,
     henüz gerçekleşmemiş] kayıtlar, (2) düzenli ödemelerin önümüzdeki
     aylara projekte edilen tarihleri, (3) kredi kartlarının hesaplanmış
     ödeme günleri (güncel borç tutarıyla), (4) kredi hesaplarının henüz
     ödenmemiş taksitleri (amortisman tablosundan).
     Başlangıç bakiyesi kasıtlı olarak cashLikeTotal() (nakit + banka +
     harici ödeme araçları + verilen borç) — kredi kartı/kredi hesaplarının
     KENDİ güncel borcu başlangıca dahil edilmiyor, çünkü bu borçların
     ödemeleri zaten aşağıda ayrı satırlar olarak vadesi geldikçe düşülüyor;
     ikisi birden sayılırsa bakiye gerçekte olduğundan daha düşük görünür. */
  function upcomingPaymentsRows(monthsAhead){
    monthsAhead = monthsAhead || 3;
    var today = todayISO();
    var horizon = addMonthsToDate(today, monthsAhead);
    var startMk = currentMonthKey();
    var rows = [];

    /* 1) Planlı kayıtlar (gelir/gider/transfer) */
    state.transactions.filter(function(t){ return isPlanned(t) && t.date<=horizon; }).forEach(function(t){
      if(t.type==='transfer'){
        var toAcct = getAccount(t.toAccountId);
        if(toAcct && isDebtType(toAcct.type)){
          rows.push({ date:t.date, kind:'expense', amount:t.amount, title:toAcct.name + ' ödemesi', source:'Planlı' });
        }
        return;
      }
      var info = txCategoryInfo(t);
      var acct = getAccount(t.accountId);
      rows.push({ date:t.date, kind:t.type, amount:t.amount, title: info.name + (t.note?(' · '+t.note):'') + (acct?(' · '+acct.name):''), source:'Planlı' });
    });

    /* 2) Düzenli ödemeler — henüz bu ay için gerçek kayıt oluşmamış olanlar,
       aktif kaldığı sürece (taksit sayısı varsa onunla sınırlı) önümüzdeki
       aylara projekte edilir. */
    state.recurring.filter(function(r){ return r.active; }).forEach(function(r){
      var maxOcc = monthsAhead + 1;
      if(r.totalInstallments!=null && r.installmentsRemaining!=null) maxOcc = Math.min(maxOcc, r.installmentsRemaining);
      for(var i=0;i<maxOcc;i++){
        var mk = addMonths(startMk, i);
        if((r.skippedMonths||[]).indexOf(mk)!==-1) continue;
        var d = mk + '-' + pad2(clamp(r.day,1,28));
        if(d<today || d>horizon) continue;
        if(state.transactions.some(function(t){ return t.id===recurringGenId(r, mk); })) continue;
        var acct2 = getAccount(r.accountId);
        rows.push({ date:d, kind:r.type, amount:r.amount, title:r.name + ' (Düzenli' + (r.isSubscription?' · abonelik':'') + ')' + (acct2?(' · '+acct2.name):''), source:'Düzenli Ödeme' });
      }
    });

    /* 3) Kredi kartı ödeme günleri — önümüzdeki her dönem için hesaplanan
       ödeme tarihi, o dönemin güncel bilinen borcuyla (basitleştirme: gelecek
       dönemlerin henüz oluşmamış harcamaları hesaba katılmıyor, yalnızca şu
       anki toplam borç kullanılıyor). */
    state.accounts.filter(function(a){ return a.type==='card' && a.statementDay && a.paymentDay; }).forEach(function(a){
      var bal = accountBalance(a.id);
      if(bal<=0) return;
      for(var i=0;i<=monthsAhead;i++){
        var mk2 = addMonths(startMk, i);
        var payDate = cardPaymentDateForStatement(a, mk2);
        if(payDate<today || payDate>horizon) continue;
        rows.push({ date:payDate, kind:'expense', amount:bal, title:a.name + ' ödemesi (kart borcu)', source:'Kredi Kartı' });
        break; /* aynı borç, ilk denk gelen ödeme gününde bir kez gösterilir */
      }
    });

    /* 4) Kredi taksitleri — amortisman tablosundaki henüz ödenmemiş satırlar */
    state.accounts.filter(function(a){ return a.type==='loan_account' && a.schedule && a.schedule.length; }).forEach(function(a){
      var paidCount = loanPaidCount(a);
      a.schedule.forEach(function(r, idx){
        if(idx<paidCount) return;
        if(r.date<today || r.date>horizon) return;
        rows.push({ date:r.date, kind:'expense', amount:r.payment, title:a.name + ' taksidi (#' + r.no + ')', source:'Kredi Taksiti' });
      });
    });

    rows.sort(function(a,b){ return a.date<b.date?-1:(a.date>b.date?1:0); });

    var balance = cashLikeTotal();
  ...
  function renderTabbar(){
    var tabs = [
      ...
      {id:'bilgi', label:'Bilgi', icon:'coin'},
  ...
  function renderTab(){
    ...
    if(ui.tab==='bilgi') return renderFinansalBilgi();
  ...
  function renderRaporlar(){
    ...
    html += renderUpcomingPaymentsSection(mk);
    html += renderInsightsSection(mk);
    html += renderSubscriptionsCard();

    var budgetByCat = {};
    budgetStatusForMonth(mk).forEach(function(b){ budgetByCat[b.cat.id] = b; });

    /* Bütçesi tanımlı ama bu ay henüz hiç gerçek harcaması olmayan kategoriler
       (ör. yalnızca henüz "Ekle" denmemiş bir abonelik borcu olan kategori)
       de burada, sıfır çubukla ama bütçe/abonelik notuyla birlikte gösterilir
       — aksi halde "beklenen abonelik" notu hiçbir yerde görünmezdi. */
    var budgetOnlyRows = budgetStatusForMonth(mk).filter(function(b){
      return !expRows.some(function(r){ return r.cat.id===b.cat.id; });
    }).map(function(b){ return { cat:b.cat, amount:0, plannedAmount:0 }; });

    html += '<div class="section-title">Gider Dağılımı</div><div class="card">';
    if(!expRows.length && !budgetOnlyRows.length){ html += renderEmpty('Bu ay gider kaydı yok.'); }
    else {
      expRows.concat(budgetOnlyRows).forEach(function(r){
  ...
  /* Gelecek Ödemeler tablosu: her satırda tarih, işlem, tutar (gelir/gider)
     ve o satıra kadar kümülatif tahmini bakiye. Yalnızca Raporlar geçerli ay
     seçiliyken gösterilir (geçmiş/başka aylarda "gelecek" kavramı anlamsız). */
  function renderUpcomingPaymentsSection(mk){
    if(mk !== currentMonthKey()) return '';
    var rows = upcomingPaymentsRows(3);
    var html = '<div class="section-title">Gelecek Ödemeler (önümüzdeki 3 ay)</div><div class="card">';
    html += '<p class="mute2" style="padding:10px 14px 0">Planlı kayıtlar, düzenli ödemeler, kredi kartı ödeme günleri ve kredi taksitleri tek bir kronolojik listede. Başlangıç bakiyesi: nakit + banka + ödeme araçları toplamınız (' + fmtTRY(cashLikeTotal()) + ') — kredi kartı/kredi borcunuzun kendisi buna dahil değil, ödemeleri vadesi geldikçe aşağıda ayrı satır olarak düşülüyor (çifte sayılmasın diye).</p>';
    if(!rows.length){
      html += renderEmpty('Önümüzdeki 3 ay için planlı/düzenli bir ödeme veya kredi/kart vadesi görünmüyor.');
    } else {
      html += '<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:0.82rem;margin-top:6px">';
      html += '<thead><tr style="position:sticky;top:0;background:var(--surface)">' +
        '<th style="padding:6px 8px;text-align:left">Tarih</th><th style="padding:6px 8px;text-align:left">Açıklama</th>' +
        '<th style="padding:6px 8px;text-align:left">Kaynak</th><th style="padding:6px 8px;text-align:right">Tutar</th>' +
        '<th style="padding:6px 8px;text-align:right">Tahmini Bakiye</th></tr></thead><tbody>';
      rows.forEach(function(r){
        var neg = r.runningBalance < 0;
        html += '<tr>' +
          '<td style="padding:6px 8px;white-space:nowrap">' + fmtDate(r.date) + '</td>' +
          '<td style="padding:6px 8px">' + esc(r.title) + '</td>' +
          '<td style="padding:6px 8px"><span class="mute2">' + esc(r.source) + '</span></td>' +
          '<td style="padding:6px 8px;text-align:right;color:' + (r.kind==='income'?'var(--income)':'var(--expense)') + '" class="num">' + (r.kind==='income'?'+':'−') + fmtTRY(r.amount) + '</td>' +
          '<td style="padding:6px 8px;text-align:right' + (neg ? ';color:var(--expense)' : '') + '" class="num">' + fmtTRY(r.runningBalance) + '</td>' +
          '</tr>';
      });
      html += '</tbody></table></div>';
    }
  ...
  /* Harcama alışkanlığı içgörüleri: yalnızca cari ay seçiliyken anlamlı
     (geçmiş ay/gelecek ay karşılaştırması için); geçmiş/gelecek aylarda
     gösterilmez. */
  function renderInsightsSection(mk){
    if(mk !== currentMonthKey()) return '';
    var insights = spendingInsights(mk);
    if(!insights.length) return '';
    var html = '<div class="section-title">Harcama Alışkanlığı İçgörüleri</div><div class="card">';
    insights.forEach(function(ins){
      var color = ins.kind==='up' ? 'var(--expense)' : (ins.kind==='down' ? 'var(--income)' : 'var(--ink)');
      html += '<div class="row" style="align-items:flex-start;gap:8px">' + icon('insight') + '<span style="color:' + color + '">' + ins.text + '</span></div>';
    });
    html += '</div>';
    return html;
  }
  ...
  function renderBudgetLine(b){
    ...
    if(b.pendingSub>0) msg += ' <span class="mute2">(+' + fmtTRY(b.pendingSub) + ' bu ayki abonelik, henüz eklenmedi)</span>';
  ...
  /* Genel tarih bazlı hatırlatıcılar (sigorta, muayene, garanti vb. —
     madde 6) + kredi kartı ödeme günü hatırlatıcıları (3/2/1 gün kala),
     tek bir banner'da birleşik. */
  function renderReminderBanner(){
    var general = upcomingReminders();
    var cards = creditCardDueSoonReminders();
    if(!general.length && !cards.length) return '';
    var html = '<div class="card recur-banner reminder-banner">';
    html += '<div class="recur-banner-head">' + icon('bell') + '<span>Hatırlatıcılar</span></div>';
    cards.forEach(function(c){
      var when = c.days===0 ? 'Bugün' : (c.days===1 ? 'Yarın' : c.days + ' gün sonra');
      html += '<div class="recur-item"><div class="recur-item-main"><div class="n">' + esc(c.account.name) + ' ödemesi</div>' +
        '<div class="s">' + when + ' (' + fmtDate(c.date) + ') · ' + fmtTRY(c.balance) + '</div></div></div>';
    });
    general.forEach(function(x){
      var when = x.days<0 ? Math.abs(x.days) + ' gün önce geçti' : (x.days===0 ? 'Bugün' : (x.days===1 ? 'Yarın' : x.days + ' gün sonra'));
      html += '<div class="recur-item"><div class="recur-item-main"><div class="n">' + esc(x.reminder.title) + '</div>' +
        '<div class="s">' + when + ' (' + fmtDate(x.date) + ')' + (x.reminder.note ? ' · ' + esc(x.reminder.note) : '') + '</div></div>' +
        (ui.readOnly ? '' : '<div class="recur-item-actions"><button class="btn btn-ghost btn-sm" data-action="dismiss-reminder" data-id="' + x.reminder.id + '">Gördüm</button></div>') +
        '</div>';
    });
    html += '</div>';
    return html;
  ...
  /* ---------------- FİNANSAL BİLGİLENDİRME ----------------
     Kullanıcı isteği: canlı/otomatik kur çekimi sisteme (Net Varlık'a vb.)
     KESİNLİKLE dahil edilmesin (manuel kur felsefesi korunuyor — madde 2),
     ama Finansal Bilgilendirme'de "bilgi amaçlı" güncel kur/altın görülebilsin.
     Veri, GitHub Actions ile HER GÜN otomatik (Claude'a veya herhangi bir
     manuel işleme bağlı olmadan) frankfurter.dev (ECB, ücretsiz/keysiz/
     limitsiz) ve gold-api.com (ücretsiz/keysiz/limitsiz) kaynaklarından
     çekilip docs/rates.json'a yazılır (bkz. scripts/fetch-rates.js ve
     .github/workflows/fetch-rates.yml). Bu sekme yalnızca o statik dosyayı
     okur — kendi başına hiçbir dış servise bağlanmaz. Yalnızca bağımsız
     sitede anlamlıdır (docs/rates.json yalnızca orada, sayfayla aynı yerde
     durur); Claude Artifact sürümünde bilgilendirici bir mesaj gösterilir. */
  function fetchFinRates(){
    if(!HAS_REAL_AUTH) return;
    ui.finRatesLoading = true;
    ui.finRatesError = null;
    render();
    fetch('./rates.json?_=' + Date.now(), { cache:'no-store' }).then(function(r){
      if(!r.ok) throw new Error('HTTP ' + r.status);
      return r.json();
    }).then(function(data){
      ui.finRates = data;
      ui.finRatesLoading = false;
      render();
    }).catch(function(err){
      ui.finRatesError = 'Kur bilgisi yüklenemedi: ' + (err && err.message ? err.message : 'bilinmeyen hata');
      ui.finRatesLoading = false;
      render();
    });
  }

  function renderFinansalBilgi(){
    var html = '<div class="section-title">Güncel Kur ve Altın Bilgisi</div>';
    if(!HAS_REAL_AUTH){
      html += '<div class="card"><p class="mute2">Bu bilgi yalnızca bağımsız site sürümünde kullanılabilir; otomatik günlük güncelleme GitHub üzerinde çalışır ve bu Claude Artifact görünümüne dahil değildir.</p></div>';
      return html;
    }
    html += '<div class="card">';
    if(ui.finRatesLoading){
      html += '<p class="mute2">Yükleniyor…</p>';
    } else if(ui.finRatesError){
      html += '<p class="mute2" style="color:var(--critical)">' + esc(ui.finRatesError) + '</p>';
      html += '<p class="mute2">Bu, günlük otomatik güncellemenin ilk kez henüz çalışmamış olmasından da kaynaklanabilir.</p>';
    } else if(ui.finRates){
      var fr = ui.finRates;
      html += '<table class="kv-table">';
      if(fr.usdTry!=null) html += '<tr><td>USD / TRY</td><td class="num">' + fr.usdTry.toFixed(4) + '</td></tr>';
      if(fr.eurTry!=null) html += '<tr><td>EUR / TRY</td><td class="num">' + fr.eurTry.toFixed(4) + '</td></tr>';
      if(fr.gramAltinTry!=null) html += '<tr><td>Gram Altın (TRY)</td><td class="num">' + fmtTRY(fr.gramAltinTry) + '</td></tr>';
      if(fr.onsAltinUsd!=null) html += '<tr><td>Ons Altın (USD)</td><td class="num">$' + fr.onsAltinUsd.toFixed(2) + '</td></tr>';
      html += '</table>';
      if(fr.updatedAt) html += '<p class="mute2" style="padding:0 14px 12px">Son güncelleme: ' + esc(fmtDateTime(fr.updatedAt)) + ' (her gün otomatik güncellenir)</p>';
    }
    html += '</div>';
    if(!ui.finRatesLoading){
      html += '<button class="btn btn-ghost btn-sm" style="margin-top:10px" data-action="finrates-refresh">' + icon('repeat') + ' Yenile</button>';
    }
    html += '<p class="mute2" style="margin-top:14px">Bu sayfadaki kur ve altın bilgisi yalnızca bilgilendirme amaçlıdır; hesaplarınızdaki döviz cinsinden bakiyeler ve Net Varlık hesaplaması hâlâ Ayarlar\'da sizin girdiğiniz manuel kuru kullanır, buradaki otomatik veriden ETKİLENMEZ.</p>';
    return html;
  }
  ...
  function renderAyarlar(){
    ...
    html += '<div class="switch-row"><span>Giriş yapan: ' + esc(me ? me.name : '—') +
      (HAS_REAL_AUTH && fbUser && fbUser.email ? ' <span class="mute2">(' + esc(fbUser.email) + ')</span>' : '') + '</span>' +
      '<button class="btn btn-ghost btn-sm" data-action="identity-logout">' + (HAS_REAL_AUTH ? 'Çıkış Yap' : 'Kullanıcı Değiştir') + '</button></div>';
    html += '</div>';
    ...
    html += renderRemindersSection();
    html += renderShareShortcutCard();

    html += '<div class="section-title">Veri</div><div class="card">';
    if(HAS_REAL_AUTH && state.accounts.length<=1 && !state.transactions.length){
    ...
    if(HAS_REAL_AUTH) html += renderBackupsSubsection();
    html += '</div>';

    html += renderAuditLogSection();

    html += '<p class="mute2" style="margin-top:18px;text-align:center">' +
      (HAS_REAL_AUTH ? 'Hane defterinizdeki tüm üyeler aynı veriyi gerçek zamanlı paylaşır.' : 'Bu sayfa yayınlandığı andan itibaren tüm görüntüleyenler aynı veriyi paylaşır.') +
      '</p>';
    return html;
  }
  ...
  /* Otomatik periyodik yedekleme (Ayarlar > Veri): her gün otomatik alınan
     yedeklerin listesi (istenince çekilir) + geri yükleme. */
  function renderBackupsSubsection(){
    var html = '<div class="row" style="margin-top:10px;border-top:1px solid var(--border);padding-top:10px"><div class="stack"><span>Otomatik Yedekler</span><span class="mute2">Her gün otomatik alınır, son 30 gün saklanır.</span></div>';
    if(ui.backupsList==null){
      html += '<button class="btn btn-sm" data-action="view-backups">Yedekleri Görüntüle</button>';
    }
    html += '</div>';
    if(ui.backupsList==='loading'){
      html += '<p class="mute2" style="margin-top:6px">Yükleniyor…</p>';
    } else if(Array.isArray(ui.backupsList)){
      if(!ui.backupsList.length){
        html += '<p class="mute2" style="margin-top:6px">Henüz otomatik yedek yok.</p>';
      } else {
        ui.backupsList.forEach(function(b){
          html += '<div class="switch-row"><span>' + esc(fmtDate(b.id)) + '</span>' +
            (ui.readOnly ? '' : '<button class="btn btn-ghost btn-sm" data-action="restore-backup" data-id="' + esc(b.id) + '">Bu Tarihe Geri Yükle</button>') +
            '</div>';
        });
      }
    }
  ...
  function renderRecurringSection(){
    ...
    if(!state.recurring.length){
      ...
      state.recurring.forEach(function(r){
        ...
        html += '<div class="switch-row"><div class="stack"><span>' + esc(r.name) + (r.isSubscription ? ' <span class="pill pill-neutral">Abonelik</span>' : '') + '</span><span class="mute2">' + sub.map(esc).join(' · ') + '</span></div>' +
  ...
  function renderRecurringForm(){
    ...
    if(f.type==='expense'){
      html += '<label class="switch-row" style="cursor:pointer"><span>Bu bir abonelik</span><input type="checkbox" name="isSubscription" value="1"' + (f.isSubscription?' checked':'') + '></label>';
    }
  ...
  /* Abonelik paneli: aktif abonelikleri ve toplam aylık maliyeti tek bakışta
     gösteren kart (Raporlar'da). Kullanıcı isteği: bu tutarların ilgili
     kategori bütçelerine ve nakit akışı projeksiyonuna yansıması — bkz.
     budgetStatusForMonth() (pendingSub) ve cashFlowProjection()
     (indefiniteRecurringNet, süresiz aboneliklerin zaten dahil olduğu yer). */
  function renderSubscriptionsCard(){
    var subs = activeSubscriptions();
    if(!subs.length) return '';
    var total = subscriptionsMonthlyTotal();
    var html = '<div class="section-title">Aktif Abonelikleriniz</div><div class="card">';
    subs.forEach(function(r){
      var acct = getAccount(r.accountId);
      var cat = getCategory('expense', r.categoryId);
      html += '<div class="switch-row"><div class="stack"><span>' + esc(r.name) + '</span><span class="mute2">' + [cat?cat.name:'', 'Her ayın ' + r.day + '. günü', acct?acct.name:'Hesap silinmiş'].filter(Boolean).map(esc).join(' · ') + '</span></div>' +
        '<span class="num">' + fmtTRY(r.amount) + '</span></div>';
    });
    html += '<div class="row" style="margin-top:8px;border-top:1px solid var(--border);padding-top:8px"><span style="font-weight:600">Toplam Aylık Abonelik Maliyeti</span><span class="num" style="font-weight:600">' + fmtTRY(total) + '</span></div>';
    html += '</div>';
    return html;
  }

  /* ---------------- Tarih bazlı genel hatırlatıcılar — yönetim (Ayarlar) ---------------- */
  function renderRemindersSection(){
    var html = '<div class="section-title">Hatırlatıcılar</div><div class="card">';
    if(!state.reminders.length){
      html += renderEmpty('Henüz bir hatırlatıcı eklenmedi (ör. sigorta yenileme, araç muayenesi, garanti bitişi).');
    } else {
      state.reminders.slice().sort(function(a,b){ return nextReminderOccurrence(a) < nextReminderOccurrence(b) ? -1 : 1; }).forEach(function(r){
        var occ = nextReminderOccurrence(r);
        var sub = [fmtDate(occ)];
        if(r.repeatYearly) sub.push('Her yıl tekrarlar');
        if(r.note) sub.push(r.note);
        html += '<div class="switch-row"><div class="stack"><span>' + esc(r.title) + '</span><span class="mute2">' + sub.map(esc).join(' · ') + '</span></div>' +
          (ui.readOnly ? '' : '<div style="display:flex;gap:4px;flex:none">' +
            '<button class="btn btn-ghost btn-sm" data-action="edit-reminder" data-id="' + r.id + '" aria-label="Düzenle">' + icon('edit') + '</button>' +
            '<button class="btn btn-ghost btn-sm" data-action="delete-reminder" data-id="' + r.id + '" aria-label="Sil">' + icon('trash') + '</button>' +
            '</div>') +
          '</div>';
      });
    }
    ...
    if(!ui.readOnly){
      if(!ui.reminderForm){
        html += '<button class="btn btn-ghost btn-sm" style="margin-top:10px" data-action="reminder-new">' + icon('plus') + ' Hatırlatıcı Ekle</button>';
      } else {
        html += renderReminderForm();
      }
    }
    return html;
  }
  ...
  function renderReminderForm(){
    var f = ui.reminderForm;
    var html = '<div class="card" style="margin-top:10px"><form data-action="save-reminder">';
    html += '<div class="field"><label>Başlık</label><input id="modal-first-input" type="text" name="title" placeholder="ör. Kasko Yenileme" value="' + esc(f.title||'') + '" required></div>';
    html += '<div class="field"><label>Tarih</label><input type="date" name="date" value="' + esc(f.date||'') + '" required></div>';
    html += '<label class="switch-row" style="cursor:pointer"><span>Her yıl tekrarlar</span><input type="checkbox" name="repeatYearly" value="1"' + (f.repeatYearly?' checked':'') + '></label>';
    html += '<div class="field"><label>Not (opsiyonel)</label><input type="text" name="note" value="' + esc(f.note||'') + '"></div>';
    html += '<input type="hidden" name="id" value="' + esc(f.id||'') + '">';
    html += '<div style="display:flex;gap:10px"><button type="button" class="btn" data-action="reminder-form-cancel">Vazgeç</button><button type="submit" class="btn btn-primary btn-block">Kaydet</button></div>';
    html += '</form></div>';
    return html;
  }
  ...
  function renderBulkImportForm(){
    ...
    if(bi.step==='input'){
      ...
      html += '<div class="field"><label>Veya bir dosyadan yükleyin</label>' +
        '<input type="file" accept=".csv,.txt" data-action="bulk-import-file" style="width:100%">' +
        '<span class="mute2">Bankanızın indirdiğiniz ekstre dosyası — yalnızca CSV/metin biçimi desteklenir; Excel dosyanız varsa "Farklı Kaydet &gt; CSV" ile kaydedip öyle yükleyin.</span></div>';
  ...
  function persist(){
    ...
    if(BACKEND === 'firebase'){
      ...
      fbHouseholdRef.update({ state: state, updatedAt: firebase.firestore.FieldValue.serverTimestamp() }).then(function(){
        fbSaveDailyBackupIfNeeded();
      }).catch(function(err){
        showToast('Kaydedilemedi: ' + (err && err.message ? err.message : 'bilinmeyen hata'));
      });
      return;
  ...
  document.body.addEventListener('click', function(e){
    ...
    if(action==='tab'){
      ...
      if(ui.tab==='bilgi' && HAS_REAL_AUTH && !ui.finRates && !ui.finRatesLoading && !ui.finRatesError){ fetchFinRates(); return; }
      render(); return;
    }
    if(action==='finrates-refresh'){ fetchFinRates(); return; }

    /* ---- otomatik yedekler ---- */
    if(action==='view-backups'){ fbListBackups(); return; }
    if(action==='restore-backup'){
      var rbId = el.getAttribute('data-id');
      ui.modal = 'confirm';
      ui.modalData = { title:'Yedeği geri yükle', message: fmtDate(rbId) + ' tarihli yedek geri yüklenecek; bu tarihten SONRA yaptığınız tüm değişiklikler kaybolacak. Bu işlem geri alınamaz.', onConfirm:{ kind:'restoreBackup', id: rbId } };
      render(); return;
    }
    ...
    if(action==='identity-logout'){
      if(HAS_REAL_AUTH){ fbSignOut(); return; }
      clearLocalIdentity();
      localIdentity = null;
      ui.identityRequesting = false;
      render(); return;
    ...
    /* ---- hatırlatıcılar ---- */
    if(action==='reminder-new'){ ui.reminderForm = { date: todayISO() }; render(); return; }
    if(action==='reminder-form-cancel'){ ui.reminderForm = null; render(); return; }
    if(action==='edit-reminder'){
      var editRem = state.reminders.find(function(x){ return x.id===el.getAttribute('data-id'); });
      ui.reminderForm = editRem ? Object.assign({}, editRem) : { date: todayISO() };
      render(); return;
    }
    if(action==='delete-reminder'){
      ui.modal = 'confirm';
      ui.modalData = { title:'Hatırlatıcıyı sil', message:'Bu hatırlatıcı kalıcı olarak silinecek.', onConfirm:{ kind:'reminder', id: el.getAttribute('data-id') } };
      render(); return;
    }
    if(action==='dismiss-reminder'){
      var dismRem = state.reminders.find(function(x){ return x.id===el.getAttribute('data-id'); });
      if(dismRem){ dismRem.dismissedOccurrence = nextReminderOccurrence(dismRem); persist(); }
      return;
    }
    ...
    if(action==='confirm-yes'){
      ...
      if(c.kind==='restoreBackup'){
        /* fbRestoreBackup() Firestore'dan ASENKRON okuyup KENDİ persist()'ini
           çağırıyor; buradaki genel (senkron) persist()'e girersek henüz
           yüklenmemiş eski state'i geri yazıp yarış durumu yaratırız — bu
           yüzden erken çıkıyoruz. */
        closeModal();
        fbRestoreBackup(c.id);
        return;
      }
      if(c.kind==='tx'){
        ...
        var delRem = state.reminders.find(function(x){ return x.id===c.id; });
        state.reminders = state.reminders.filter(function(x){ return x.id!==c.id; });
        if(delRem) logAudit('reminder-delete', 'Hatırlatıcı silindi: ' + delRem.title);
      }
  ...
  document.body.addEventListener('change', function(e){
    ...
    if(e.target.matches('[data-action="bulk-import-file"]') && e.target.files && e.target.files[0]){
      var biFile = e.target.files[0];
      var biReader = new FileReader();
      biReader.onload = function(){
        var biTa2 = document.getElementById('bulk-import-textarea');
        if(biTa2) biTa2.value = String(biReader.result||'');
        ui.bulkImport = Object.assign({}, ui.bulkImport, { text: String(biReader.result||'') });
        showToast('Dosya yüklendi: ' + biFile.name + ' — "Önizle"ye basarak devam edin.');
      };
      biReader.onerror = function(){ showToast('Dosya okunamadı, tekrar deneyin.'); };
      biReader.readAsText(biFile, 'utf-8');
    }
  });
  ...
  document.body.addEventListener('submit', function(e){
    ...
    if(action==='save-recurring'){
      ...
      var recIsSubscription = recType==='expense' && fd.get('isSubscription')==='1';
      ...
      state.recurring.push({
        ...
        isSubscription: recIsSubscription
      });
    ...
    if(action==='save-reminder'){
      var remTitle = (fd.get('title')||'').trim();
      var remDate = fd.get('date') || '';
      var remRepeat = fd.get('repeatYearly')==='1';
      var remNote = (fd.get('note')||'').trim();
      var remId = fd.get('id');
      if(!remTitle || !remDate){ showToast('Lütfen başlık ve tarih girin.'); return; }
      if(remId){
        var ridx = state.reminders.findIndex(function(x){ return x.id===remId; });
        if(ridx>-1) state.reminders[ridx] = Object.assign({}, state.reminders[ridx], { title:remTitle, date:remDate, repeatYearly:remRepeat, note:remNote, dismissedOccurrence:null });
        logAudit('reminder-edit', 'Hatırlatıcı güncellendi: ' + remTitle);
      } else {
        state.reminders.push({ id: uid('rem'), title:remTitle, date:remDate, repeatYearly:remRepeat, note:remNote, dismissedOccurrence:null });
        logAudit('reminder-add', 'Hatırlatıcı eklendi: ' + remTitle);
      }
      ui.reminderForm = null;
      persist();
      return;
    }
